...
class Begin : View("Monopoly"){
    ...
    fun active3(){
        if (player3Check.isSelected) {
            ...
            botCheck2.disableProperty().value = false
        }
        else {
            playfield3.disableProperty().value = true
            playfield3.text = ""
            botCheck2.disableProperty().value = true
            botCheck2.isSelected = false
        }
    }
    ...
    fun startGame(){
        ...
        if (botCheck1.isSelected) game.data[1].ai = true

        if (player3Check.isSelected){
            game.data.add(Player(game.data.size + 1))
            game.data.last().name = playfield3.text
            if (botCheck2.isSelected) game.data.last().ai = true
        }
